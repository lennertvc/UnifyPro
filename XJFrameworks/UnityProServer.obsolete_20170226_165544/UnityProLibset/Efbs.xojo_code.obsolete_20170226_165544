...
Inherits COM.IDispatch
	#tag Method, Flags = &h0
		Sub Constructor(connectEvents As Boolean = False)
		    ...
		    riid = COM.IIDFromString( "{4F5675BB-3732-11D6-A291-00062999F09F}" )
		    clsid = COM.IIDFromString( "{4F5675BC-3732-11D6-A291-00062999F09F}" )
	...
	#tag Method, Flags = &h0
		Shared Function IID() As MemoryBlock
		  Return COM.IIDFromString("{4F5675BB-3732-11D6-A291-00062999F09F}")
		End Function
	#tag EndMethod

	#tag Method, Flags = &h0
		Function Item(p_EfbKey_Param As Variant) As UnityProLibset.IEfb
		  If mThis = Nil Then Raise New NilObjectException
		  Dim func As New Item_Func2(mThis.Ptr( 0 ).Ptr(36 ))
		  Dim resultCode As Integer
		  Dim Local_p_EfbKey_Param As MemoryBlock = COM.RBVariantToVARIANT(p_EfbKey_Param)
		  Dim Return_pEfb_Param As Ptr
		  resultCode = func.Invoke(mThis, Local_p_EfbKey_Param, Return_pEfb_Param)
		  COM.FreeVARIANT(Local_p_EfbKey_Param)
		  If resultCode = 0 Then
		    If Nil = Return_pEfb_Param  Then Return Nil
		    Return New UnityProLibset.IEfb(Return_pEfb_Param)
		  Else // Throw Exception
	...
	#tag DelegateDeclaration, Flags = &h1
		Protected Delegate Function Item_Func2(this As Ptr, p_EfbKey_Param As Ptr, ByRef pEfb_Param As Ptr) As Integer
	#tag EndDelegateDeclaration
	...
	#tag Method, Flags = &h0
		Sub Operator_Convert(rhs As COM.IUnknown)
		  ...
		  Dim p As Ptr
		  If 0 = rhs.QueryInterface( UnityProLibset.IEfbs.IID, p ) Then
		    mThis = p
		  Else
