...
Inherits COM.IDispatch
	...
	#tag Method, Flags = &h0
		Function LCItem(p_key_Param As Variant) As UnityProLibset.IFamily
		  If mThis = Nil Then Raise New NilObjectException
		  Dim func As New LCItem_Func2(mThis.Ptr( 0 ).Ptr(48 ))
		  Dim resultCode As Integer
		  
		  
		  //Dim Local_p_key_Param As MemoryBlock = COM.RBVariantToVARIANT(p_key_Param)
		  Dim Local_p_key_Param As UnityEnum = VariantToUnityEnum(p_key_Param)
		  Dim Return_pFamily_Param As Ptr
		  resultCode = Func.Invoke(mThis, Local_p_key_Param, Return_pFamily_Param)
	...
	#tag DelegateDeclaration, Flags = &h1
		Protected Delegate Function LCItem_Func2(this As Ptr, p_key_Param As UnityEnum, ByRef pFamily_Param As Ptr) As Integer
	#tag EndDelegateDeclaration
