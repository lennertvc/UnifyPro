...
Inherits COM.IDispatch
	#tag Method, Flags = &h0
		Sub Constructor(connectEvents As Boolean = False)
		    ...
		    riid = COM.IIDFromString( "{92A2EDD9-1A24-11D5-B23A-0002553484A6}" )
		    clsid = COM.IIDFromString( "{92A2EDD7-1A24-11D5-B23A-0002553484A6}" )
	...
	#tag DelegateDeclaration, Flags = &h1
		Protected Delegate Function Item_Func3(this As Ptr, DfbName_Param As Ptr, Id_Param As Integer, ByRef ppReturnedDfb_Param As Ptr) As Integer
	#tag EndDelegateDeclaration

	#tag Method, Flags = &h0
		Function LCItem(DfbName_Param As String, Id_Param As Integer = 0) As UnityProServer.IDfb
		  If mThis = Nil Then Raise New NilObjectException
		  Dim func As New LCItem_Func3(mThis.Ptr( 0 ).Ptr(32 ))
		  Dim resultCode As Integer
		  Dim Local_DfbName_Param As UnityEnum = VariantToUnityEnum(DfbName_Param)
		  //Local_DfbName_Param = COM.SysAllocString( DfbName_Param )
		  Dim Return_ppReturnedDfb_Param As Ptr
		  resultCode = func.Invoke(mThis, Local_DfbName_Param, Id_Param, Return_ppReturnedDfb_Param)
		  //COM.SysFreeString(Local_DfbName_Param)
	...
	#tag DelegateDeclaration, Flags = &h1
		Protected Delegate Function LCItem_Func3(this As Ptr, DfbName_Param As UnityEnum, Id_Param As Integer, ByRef ppReturnedDfb_Param As Ptr) As Integer
	#tag EndDelegateDeclaration
